...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Get unread pill from interactions.js
...
✅ To verify or contact admin, use the “Contact Admin” button below.`;
  ...
  document.addEventListener("sendMessage", (ev) => {
    ...
    if(text.toLowerCase().includes("admin") || text.toLowerCase().includes("contact")){
      ...
      setTimeout(()=> { 
        window.TGRenderer && window.TGRenderer.appendMessage(admin, "Thanks — please use the Contact Admin button in the pinned banner. We'll respond there.", { timestamp: new Date(), type:"outgoing" }); 
  ...
  // Wrap TGRenderer.appendMessage to increment unread for any incoming message
  if(window.TGRenderer){
    ...
    window.TGRenderer.appendMessage = function(persona, text, opts){
      ...
      originalAppend.call(window.TGRenderer, persona, text, opts);
