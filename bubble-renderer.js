// bubble-renderer-typing-queue.js
(function(){
  function init(){
    ...
    function processTypingQueue(){
      if(typingActive || TYPING_QUEUE.length===0) return;
      typingActive = true;

      const batch = TYPING_QUEUE.splice(0,3);
      const names = batch.map(p=>p.name);
      showTypingInHeader(names);

      const bubbles = batch.map(p=>createTypingBubble(p));
      const maxDuration = Math.max(...batch.map(p=>p.typingSpeed||1200));

      setTimeout(()=>{
        bubbles.forEach(b=>b.parentNode && b.parentNode.removeChild(b));
        typingActive = false;
        processTypingQueue();
      }, maxDuration + 200);
    }
    ...
    function createBubbleElement(persona, text, opts={}){
      ...
      const timeSpan = document.createElement("span"); timeSpan.textContent=formatTime(timestamp);
      meta.appendChild(timeSpan);
      if(type==="outgoing"){
        const seen = document.createElement("div"); seen.className="seen";
        seen.innerHTML=`<i data-lucide="eye"></i> 1`;
        meta.appendChild(seen);
      }
      content.appendChild(meta);

      const reactions = document.createElement("div"); reactions.className="tg-reactions";
    ...
    window.TGRenderer = window.TGRenderer || {
      ...
      showTyping:(persona,duration=1600)=>{ enqueueTyping(persona); document.dispatchEvent(new CustomEvent("headerTyping",{detail:{name:persona?.name||"Someone"}})); }
    };

    window.BubbleRenderer = window.BubbleRenderer || {
      renderMessages:(arr=[])=>{ arr.forEach(m=>appendMessage({name:m.name,avatar:m.avatar||"https://ui-avatars.com/api/?name=U"}, m.text, {id:m.id,timestamp:new Date(m.time||Date.now()),type:m.isOwn?"outgoing":"incoming"})); }
    };

    if(window.lucide?.createIcons) try{ window.lucide.createIcons(); }catch(e){}

    console.log("bubble-renderer initialized with header + feed typing queue");
  }
